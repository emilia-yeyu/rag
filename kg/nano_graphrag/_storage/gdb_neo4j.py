...
class Neo4jStorage(BaseGraphStorage):
    def __post_init__(self):
        # 优先使用addon_params，然后使用环境变量作为默认值
        self.neo4j_url = self.global_config["addon_params"].get("neo4j_url", neo4j_url)
        self.neo4j_auth = self.global_config["addon_params"].get("neo4j_auth", (neo4j_username, neo4j_password))
        ...
        # 检查配置是否有效
        if self.neo4j_url is None or self.neo4j_url == "bolt://localhost:7687":
            logger.warning("Using default Neo4j URL. Make sure Neo4j is running locally or set NEO4J_URI environment variable.")
        
        if isinstance(self.neo4j_auth, tuple) and self.neo4j_auth[1] == "your_password":
            logger.warning("Using default Neo4j password. Please set NEO4J_PASSWORD environment variable.")
        ...
        try:
            ...
            raise ValueError(f"Neo4j connection failed. Please check your configuration: {e}")
